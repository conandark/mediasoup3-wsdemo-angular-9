...
export class XxxHomePageComponent {
  ...
  initMediaSoup() {
    ...
    const socket = new WebSocket("wss://inet.ufu.life");
    ...
    const createProducers = async () => {
      ...
      const localVideoNode: any = document.getElementById('localVideo');
    ...
    const handleConsumeResponse = async ({ consumerData }) => {
      ...
      const remoteVideoNode: any = document.getElementById(`remoteVideo-${consumerData.peerId}`);
      ...
      const videoNode: any = document.createElement('video');
    ...
    const destroySession = () => {
      ...
      let videoes: any = document.querySelector('video');
      for (const videoNode of videoes) {
        videoNode.remove();
      }
    };
